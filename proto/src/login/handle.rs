    ...
    ($conn:ident, $pk:ident, $pk_name:expr, $provider:ident, $handler:ident) => ({
        ...
        let packet = match $conn.recv().await {
            ...
            Ok(mut vec) => { match vec.into_iter().next() {
                None => { return Err(LoginError::FormatError(format!("Recieved empty gamepacket vec, at {}", $pk_name))) }
                Some(v) => { v }
            } }
        ...
        match packet {
            GamePacket::$pk(mut packet) => {
                ...
                match $provider.$handler(&mut packet) {
                    ...
                    LoginProviderStatus::AbortLogin{ reason } => { return Err(LoginError::Abort { reason }); }
                }
            }
            other => { return Err(LoginError::FormatError(format!("Expected {}, got {:?}", $pk_name, other))) }
        }
    });
}
