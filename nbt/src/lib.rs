...
impl NbtTag {
    ...
    pub fn nbt_serialize<T: NbtByteOrder>(&self, key: impl Into<String>, buf: &mut Vec<u8>) -> Result<(), NbtError> {
        match self {
            NbtTag::Byte(v) => {
    ...
    pub fn nbt_deserialize<T: NbtByteOrder>(cursor: &mut Cursor<Vec<u8>>) -> Result<(String, Self), NbtError> {
