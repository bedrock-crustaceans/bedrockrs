...
macro_rules! to_nbt_error {
    ($err:expr) => {
        match $err.kind() {
            ErrorKind::UnexpectedEof => { return Err(NbtError::UnexpectedEOF) }
            _ => { return Err(NbtError::IOError($err.to_string())) }
        }
    };
}
...
impl NbtByteOrder for NbtLittleEndianNetwork {
    ...
    fn write_string(buf: &mut Vec<u8>, string: String) -> Result<(), NbtError> {
        match Self::write_i16(buf, match string.len().try_into() {
            Ok(v) => { v },
    ...
    fn read_string(buf: &mut Cursor<Vec<u8>>) -> Result<String, NbtError> {
        ...
        let mut string_buf = vec![0; match len.try_into() {
            Ok(v) => { v }
            Err(e) => { return Err(NbtError::IntError(e)) }
        }];
